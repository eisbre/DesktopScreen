...
void http_weather_get(){
    ...
    //判断是否有存储城市信息
    if(city != NULL){
        char url[] = "http://api.seniverse.com/v3/weather/now.json?key=SWnqoXsuLIWUfPlOD&location=";
        ...
        strcpy(count_url, "http://api.seniverse.com/v3/weather/now.json?key=SWnqoXsuLIWUfPlOD&location=guangzhou&language=zh-Hans&unit=c");
    }
    //http client配置
